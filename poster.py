...
def generate_lineup():
    ...
    # --- TODAY'S HOF OVERRIDE (Hardcoded to prevent CSV lookup errors) ---
    if today == date(2026, 5, 30):
        lineup_pool = [{"Player": p} for p in ["Lee Mazzilli", "Gary Carter", "Keith Hernandez", "Edgardo Alfonzo", "David Wright", "Bud Harrelson", "Cleon Jones", "Darryl Strawberry", "Rusty Staub"]]
        defense_map = {"Lee Mazzilli": "CF", "Gary Carter": "C", "Keith Hernandez": "1B", "Edgardo Alfonzo": "2B", "David Wright": "3B", "Bud Harrelson": "SS", "Cleon Jones": "LF", "Darryl Strawberry": "RF", "Rusty Staub": "DH"}
        starter = {"Name": "Tom Seaver"}
        bp = [{"Name": "Jerry Koosman"}, {"Name": "Tug McGraw"}, {"Name": "John Franco"}, {"Name": "Dwight Gooden"}]
        bench = ["Mookie Wilson", "Ed Kranepool", "Howard Johnson", "Kevin McReynolds"]
        return lineup_pool, defense_map, starter, bp, bench, "Bobby Valentine (In Disguise) 🥸", 95

    # --- STANDARD LOGIC (For tomorrow and beyond) ---
    else:
...
def post_to_bluesky():
    try:
        ...
        for p in lineup:
            name = p['Player']
            pos = defense.get(name, "N/A")
            post_text += f"{name} {pos}\n"
        post_text += f"\nP: {starter['Name']}"

        bp_names = ", ".join([p['Name'] for p in bp_rows])
        bench_names = ", ".join(bench) if isinstance(bench, list) else "None"
