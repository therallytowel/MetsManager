...
def generate_lineup():
    ...
    pos_master = pos_df.groupby('Player')[['C', '1B', '2B', '3B', 'SS', 'LF', 'CF', 'RF']].sum().reset_index()
    pos_master['EligiblePositions'] = pos_master.apply(lambda r: [p for p in ['C', '1B', '2B', '3B', 'SS', 'LF', 'CF', 'RF'] if r[p] > 0], axis=1)
